...
# Vanta X Enterprise FMCG Platform - AWS Ubuntu 24.04 Deployment Script (Fixed)
...
install_application() {
    ...
    if [[ -f "enterprise-backend.js" && -f "enterprise-frontend.html" && -f "enterprise-app.js" ]]; then
        ...
        wget -q -O $INSTALL_DIR/enterprise-backend.js https://raw.githubusercontent.com/Reshigan/enterprise-trade-marketing-platform/enterprise-aws-deployment/enterprise-backend.js
        wget -q -O $INSTALL_DIR/index.html https://raw.githubusercontent.com/Reshigan/enterprise-trade-marketing-platform/enterprise-aws-deployment/enterprise-frontend.html
        wget -q -O $INSTALL_DIR/enterprise-app.js https://raw.githubusercontent.com/Reshigan/enterprise-trade-marketing-platform/enterprise-aws-deployment/enterprise-app.js
        
        if [[ ! -f "$INSTALL_DIR/enterprise-backend.js" ]]; then
            print_error "Failed to download application files"
            exit 1
        fi
    fi
...
EOF
        ...
        # Start CloudWatch agent (if AWS credentials are configured)
        if aws sts get-caller-identity &> /dev/null; then
            ...
            print_warning "AWS credentials not configured - CloudWatch monitoring disabled"
        fi
    fi
}
